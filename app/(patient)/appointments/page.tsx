...
import { AppointmentCard, AppointmentTableRow, type Appointment } from '../../../components/scheduling/AppointmentCard'
    ...
    <main className="pip-appointments-page">
      ...
      {appointments && appointments.length > 0 ? (
        <section aria-label="Appointments" data-testid="appointment-list">
          <div className="pip-appointment-cards">
            {appointments.map((appointment) => <AppointmentCard key={appointment.id} appointment={appointment} onUpdated={replace} />)}
          </div>
          <table className="pip-appointment-table">
            ...
            <tbody>{appointments.map((appointment) => <AppointmentTableRow key={appointment.id} appointment={appointment} onUpdated={replace} />)}</tbody>
          </table>
        </section>
      ) : null}
      <style>{`
        .pip-appointments-page { max-width: 72rem; margin: 0 auto; overflow-wrap: anywhere; }
        .pip-appointment-cards { display: grid; gap: 0.75rem; }
        ...
        .pip-appointment-card > span, .pip-appointment-card > time { display: block; }
        ...
        .pip-appointment-table { display: none; width: 100%; border-collapse: collapse; }
        .pip-appointment-table th, .pip-appointment-table td { padding: 0.75rem; text-align: left; vertical-align: top; border-bottom: 1px solid var(--pip-color-base-300); }
        .pip-appointment-table td:first-child > time, .pip-appointment-table td:first-child > span { display: block; margin-bottom: 0.25rem; }
        @media (min-width: 768px) { .pip-appointment-cards { display: none; } .pip-appointment-table { display: table; } }
      `}</style>
    </main>
  )
}
