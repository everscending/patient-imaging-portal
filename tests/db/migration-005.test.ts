...
function fixture(): { providerId: string; appointmentId: string; callerId: string } {
  fixtureSequence += 1
  const callerId = psql(run.dbName, `insert into auth.users default values returning id;`)
  const providerId = psql(run.dbName, `insert into providers (user_id,full_name,time_zone) values ('${callerId}','Provider','America/Chicago') returning id;`)
...
describe('apply_provider_availability transactional accept-and-flag', () => {
  test('removes only free in-range slots, preserves the booking, and self-heals on restore', () => {
    ...
    expect(apply(providerId, callerId, closedHours, `array['[2026-08-17 17:00+00,2026-08-17 17:30+00)']`)).toBe('1|1|1')
    expect(psql(run.dbName, `select status || '|' || out_of_hours from appointments where id='${appointmentId}';`)).toBe('confirmed|true')
    expect(psql(run.dbName, `select action || '|' || target_id::text from audit_events where actor_ref='${callerId}' order by id;`)).toBe(
      `availability.update|${providerId}\navailability.collision|${appointmentId}`,
    )
    ...
    expect(apply(providerId, callerId, restoredHours, `array['[2026-08-17 15:00+00,2026-08-17 15:30+00)','[2026-08-17 15:30+00,2026-08-17 16:00+00)']`)).toBe('1|1|0')
  ...
  test('a different slot length skips a proposal overlapping the surviving booked slot', () => {
    ...
    expect(() => apply(providerId, callerId, hours, `array['[2026-08-17 14:00+00,2026-08-17 15:00+00)','[2026-08-17 15:00+00,2026-08-17 16:00+00)','[2026-08-17 16:00+00,2026-08-17 17:00+00)']`, 60)).not.toThrow()
  ...
  test('a failure after replacement begins rolls every availability change back', () => {
    ...
    const auditBefore = psql(run.dbName, `select count(*) from audit_events where actor_ref='${callerId}';`)
    const overlapping = `[{"weekday":1,"startsLocal":"09:00","endsLocal":"13:00"},{"weekday":1,"startsLocal":"12:00","endsLocal":"17:00"}]`
    expect(() => apply(providerId, callerId, overlapping, `array[]`)).toThrow()
    expect(psql(run.dbName, `select count(*) || '|' || min(starts_local)::text from working_hours where provider_id='${providerId}';`)).toBe(before)
    expect(psql(run.dbName, `select count(*) from audit_events where actor_ref='${callerId}';`)).toBe(auditBefore)
  })
})
