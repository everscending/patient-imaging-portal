...
describe('JOR-289 live E3/E4 imaging fixture', () => {
  test('acceptance: seeded patient receives completed evidence while scheduled and cancelled successor studies remain identifiable', async () => {
    ...
    expect(studies.map((study) => study.id)).toContain(E3_SCHEDULED_STUDY_ID)
    expect(studies.map((study) => study.id)).toContain(E4_CANCELLED_STUDY_ID)
  ...
  test('mandatory adversarial: patientAndForeignIsolationKeepsCineFramesCallerScoped', async () => {
    ...
    const [seededFrames, foreignFrames] = await Promise.all([
  ...
  test('mandatory adversarial: preliminaryReportValuesSignedOnlyExclusionAndGuardLookupBehavior', async () => {
    const token = await linkedPatientToken('PT-4471')
    const [preliminary] = await patientRows(`/rest/v1/reports?id=eq.${E4_PRELIMINARY_REPORT_ID}`, token)
  ...
  test('mandatory adversarial: mutableIdentityResetPreservesStableEvidence', async () => {
    ...
    expect(await patientRows(`/rest/v1/cine_frames?clip_id=eq.${E2_SEEDED_CLIP_ID}`, firstToken)).toHaveLength(100)
    ...
    const [frames, preliminary] = await Promise.all([
      ...
      patientRows(`/rest/v1/reports?id=eq.${E4_PRELIMINARY_REPORT_ID}`, resetToken),
    ])
    expect(frames.map((frame) => frame.frame_index)).toEqual(Array.from({ length: 100 }, (_, index) => index))
    expect(preliminary).toEqual([expect.objectContaining({ id: E4_PRELIMINARY_REPORT_ID, signed_at: null })])
  })
})
