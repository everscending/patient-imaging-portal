...
// JOR-320 memoized generateAssetPool's result per seed — assets.ts's bytes
// changed, but the pool it produces did not (pinned by
// tests/seed/assets.test.ts's byte-identical-across-runs tests), so no data
// migrates here, only the checksum.
// tests/deploy/provisioning.test.ts pins it to the current checkout.
export const JOR320_MEMOIZED_POOL_SEED_CHECKSUM = '12046e4e08dbd27985caa0fa38c63a8e914f67c340212940f500780520ef27ed'
...
const SEED_DATA_CHECKSUM_UPGRADES = new Map<string, SeedDataUpgrade>([
  ...
  [
    ...
    { to: JOR320_MEMOIZED_POOL_SEED_CHECKSUM, apply: upgradeMemoizedPoolChecksumOnly },
  ],
])
...
/** JOR-320: memoizing generateAssetPool changed no generated byte (only
 *  assets.ts's own source), so this step is a checksum bump with no data
 *  to touch — same shape as upgradeLegacySeedChecksum above. */
function upgradeMemoizedPoolChecksumOnly(sql: typeof runPsql, marker: NonNullable<SeedMarker>, markerChecksum: string, checksum: string): void {
  sql(`do $$
begin
  update app_deploy.seed_runs
     set checksum = ${sqlLiteral(checksum)}
   where singleton and source_seed = ${sqlLiteral(marker.sourceSeed)} and checksum = ${sqlLiteral(markerChecksum)};
  if not found then
    raise exception 'seed marker changed while its checksum contract was upgraded';
  end if;
end $$;`)
